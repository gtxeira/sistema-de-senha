...
export default function DashboardPage() {
    ...
    const channel = db
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!call || call.number_int == null) return;
          ...
          const callTypeFormatted =
            call.type === "preferential" || call.type === "preferencial"
              ? "preferencial" : "normal";
          const field = callTypeFormatted === "preferencial" ? "priorityCurrent" : "normalCurrent";
    ...
    await withQueueLock(async () => {
      ...
      try {
        const response = await fetch("/api/queue/call", {
          ...
          body: JSON.stringify({ sector, type, attendantId: session?.id || null }),
        });
  ...
  return (
    <main className={`${styles.shell} ${session?.accessLevel === 2 ? styles.secondary : ""}`}>
      ...
      <section className={styles.main}>
        <header className={styles.header}>
          <div>
            ...
            <h1>Olá, {session?.name || "Atendente"} <span>👋</span></h1>
            <p className={styles.muted}>Controle as chamadas da sua unidade em tempo real.</p>
          </div>
